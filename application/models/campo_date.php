...
class CampoDate extends Campo{
    ...
    protected function display($modo, $dato, $etapa_id) {
        ...
        $display.='<input id="'.$this->id.'" class="datepicker" ' . ($modo == 'visualizacion' ? 'disabled' : '') . ' type="text" name="'.$this->nombre.'" value="' . ($dato && $dato->valor?date('d-m-Y',strtotime($dato->valor)):$valor_default) . '" placeholder="dd-mm-aaaa" />';
        if($this->ayuda)
