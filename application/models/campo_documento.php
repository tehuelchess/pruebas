...
class CampoDocumento extends Campo {
    ...
    private function displayFirmador($modo, $dato, $etapa_id) {
        ...
        $isLinux = stripos( $_SERVER['HTTP_USER_AGENT'] , 'Linux' ) !== false;
                ...
                function checkPasswordToken(){
                    var passwordToken = $("#passwordTokenValue").val();
                            ...
                            if (value === "true") {
                                ...
                                alert("No se ha detectado Token, por favor inserte su Token de firma o la contraseña ingresada es la incorrecta");
                            }
                }
            </script>
            <div id="password">
        ...
        $display .= '
            <div id="verificarFirma">
                <input id="estaFirmado" type="text" maxlength="0" required style="visibility:hidden;" value=""/>
            </div><br/>
            <script>
                function firmarConToken(){                  
                    ...
                    if (status==="true") {
                        ...
                        document.getElementById("estaFirmado").value="Documento firmado con éxito";
                    }else{
            ...
            <applet code="'.($isMac?'appletFirma.SimpleFirmaMAC':($isLinux? 'appletFirma.SimpleFirmaLinux':'appletFirma.SimpleFirmaWS')).'" width="350" height="25" name="SignerApplet">
                <param name="jnlp_href" value="'.base_url().'assets/applets/signer/'.($isMac?'SignerAppletSimple_1_0_mac.jnlp':($isLinux?'SignerAppletSimple_1_0_linux.jnlp':'SignerAppletSimple_1_0_win.jnlp')).'" />
                <param name="documentosPdf" value="'.  htmlspecialchars('<PorFirmar><documento id=\''.$file->id.'\' token=\''.$file->llave_firma.'\' comentario=\'Firmado Digitalmente\' lugar=\'Asunción\' tipoFirma=\'TIPO_DOC\'/></PorFirmar>').'" />
