...
class UsuarioSesion {
    ...
    private static function login_open_id() {
        $CI = & get_instance();
        if ($CI->lightopenid->validate() && strpos($CI->lightopenid->claimed_id, 'http://gdl.senatics.gov.py/identidadelectronica') === 0) {
            ...
            if (!$usuario) {
                ...
                /* codigo agregado para el almacenamiento automatico de los datos del server open id */
                $usuario->nombres = $atributos['namePerson/first'];
                $usuario->apellido_paterno = $atributos['namePerson/last'];
                $usuario->apellido_materno = $atributos['namePerson/secondLast'];
