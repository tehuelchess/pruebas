...
class Recursos extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $cms->setAccount(UsuarioBackendSesion::usuario()->cuenta_id);
        $cms->loadData();
        
        $alfresco = new Alfresco();
        $this->alfresco = $alfresco;
        $this->_cms = $cms;
        $this->pathFolder = $this->_cms->getRootFolder() . '/RESOURCES';
    }
    
    public function index()
    {
        $data['title'] = 'Recursos Generales';
        $data['content'] = 'backend/recursos/index';
    ...
    {        
        ...
        $url_view = site_url('backend/recursos/getfile?base=' . $nodeRef . '&filename=' . $data['nombre']);
        
        $data['urlfile'] = $url_view;
        $data['noderef'] = $data['id'];
        
        $this->load->view('backend/recursos/ajax_editar', $data); 
    }
            ...
            if (isset($data['checked']) && $data['checked'] == 1) {
                ...
                if (count($data) > 0) {                    
                    foreach($data as $item) {
                        ...
                        $url_view = site_url('backend/recursos/getfile?base=' . $base . '&filename=' . $item->nombre);
