...
<div class="row-fluid">
    <div class="span9">
        <ul class="breadcrumb">
            <li>
                <a href="<?=site_url('manager/recursos')?>">Configuración</a> <span class="divider">/</span>
            </li>
...
<script>
    ...
    $(document).ready(function() {
                    ...
                    if ($.trim($("#cmbtipo").val()) != "") {
                        if ($.trim($("#txtdescripcion").val()) != "") {
                            ...
                            if (!idExiste) {
                                ...
                                var url = "<?= site_url('manager/recursos/save/') ?>";
                                $('#namefile').val(file);
                                $.ajax({
                                    url: url,
        ...
        $(document).on('click', '.js-editar-recurso', function() {
            ...
            $("#modaleditresources").load("<?= site_url('manager/recursos/ajaxviewedit/') ?>" + '?' + param);
        ...
        $(document).on('click', '.js-save-edit-recur', function(){
            $('.validacion').html('');            
            var identificador = $.trim($("#edit_txtidentificador").val());
            var noderef = $.trim($("#edit_txtnoderef").val());
                        
            if (identificador != "") {
                if ($.trim($("#edit_txtdescripcion").val()) != "") {
                    var idExiste = false;
                    ...
                    if (!idExiste) {
                        ...
                        var url = "<?= site_url('manager/recursos/ajaxsaveedit/') ?>";                        
                        $.ajax({
                            url: url,                            
                            data: {noderef: $('#edit_txtnoderef').val(), identificador: $('#edit_txtidentificador').val(), tipo: $('#edit_cmbtipo').val(), descripcion: $('#edit_txtdescripcion').val()},
                            dataType: "json",
                            success: function(data) {
                                if ($.trim(data.status) == "success") {
                                    $('.ajaxLoader').remove();
    ...
    function eliminar_archivo_simple(nombre) {        
        $.ajax({
            url: "<?= site_url('manager/recursos/deletefile') ?>",
            data: {
                file: nombre
            },
            dataType: "json",
    ...
    function eliminar_archivo(nombre, obj) {        
        $.ajax({
            url: "<?= site_url('manager/recursos/ajaxdelete') ?>",            
            data: {
                nombre: nombre
            },
            dataType: "json",
    ...
    function ajax_cargar_resources() {        
        ...
        $.ajax({
            url: "<?= site_url('manager/recursos/ajaxview') ?>",
            dataType: "json",
            success: function(data) {                
