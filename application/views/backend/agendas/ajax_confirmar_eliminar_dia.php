...
<div class="modal-header">
    ...
    <h3 id="myModalLabel">Eliminar dia</h3>
</div>
    <div class="modal-body">
        <div class="validacion"></div>
        <?php
            if(isset($selecciono) && $selecciono==1){
                echo '<label>Esta seguro de querer eliminar el dia <strong><span id="diaeliminar"></span></strong> .</label>';
            }else{
...
<div class="modal-footer">
    ...
    <?php
        if(isset($selecciono) && $selecciono==1){
            echo '<a href="#" onclick="ajax_eliminar_cita(\''.$fecha.'\');" class="btn btn-primary">Eliminar dia</a>';
        }
    ?>
...
<script>
    ...
    function ajax_eliminar_cita(fecha){
        ...
        var url='<?=site_url('/backend/agendas/ajax_eliminar_dia_feriado')?>';
        var id=<?= $id ?>;
        $.ajax({
            url: url,
            data:{
            ...
            success: function( data ) {
                if(data.code==201 || data.code==200){
                   location.href='<?=site_url('/backend/configuracion/feriados')?>';
                }else{
